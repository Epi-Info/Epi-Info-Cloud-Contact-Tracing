    ...
    public class SurveyController : BaseSurveyController
    {
        ...
        public ActionResult Index(string responseId, int PageNumber = 1, string Edit = "", string FormValuesHasChanged = "", string surveyid = "")
        {
            try
            {
                ...
                string _rootResponseId = (Session[SessionKeys.RootResponseId] ?? string.Empty).ToString();                
                if (_rootResponseId == responseId)
                {
        ...
        private SurveyModel GetIndex(string responseId, int PageNumber, string Edit, string SurveyId, bool IsAndroid = false)
        {
            ...
            {
                Session[SessionKeys.RequestedViewId] = FormsHierarchy.Where(x => x.FormId == SurveyId).Select(x => x.ViewId).First();
                Session[SessionKeys.IsSqlProject] = FormsHierarchy.Where(x => x.FormId == SurveyId).Select(x => x.IsSqlProject).First();
            }
            int _requestedViewId;
            int.TryParse((Session[SessionKeys.RequestedViewId] ?? 0).ToString(), out _requestedViewId);
            RelateSurveyId = FormsHierarchy.Single(x => x.ViewId == _requestedViewId).FormId;            
            ...
            //if (Session[SessionKeys.RequestedViewId] != null)
            //{
            ...
            switch (ValidationTest)
            {
                ...
                default:
                    ...
                    form.FormValuesHasChanged = (Session[SessionKeys.FormValuesHasChanged] ?? string.Empty).ToString();
        ...
        private void UpdateStatus(string ResponseId, string SurveyId, int StatusId)
        {
            ...
            SurveyAnswerRequest.Criteria.IsShareable = true;
                ...
                switch (ValidationTest)
                {
                    ...
                    default:
                        ...
                        if (!string.IsNullOrEmpty(this.Request.Form["is_save_action"]) && this.Request.Form["is_save_action"].ToString().Equals("true", StringComparison.OrdinalIgnoreCase))
                        {
                            ...
                            SaveCurrentFormToDocumentDB(DocumentForm, surveyInfoModel, SurveyAnswer, responseId, UserId, IsSubmited, IsSaved, FormValuesHasChanged, PageNumber, responseId);
                            form = SetLists(form);
                            TempData["Width"] = form.Width + 5;
                            SurveyModel SurveyModel = new SurveyModel();
                            SurveyModel.Form = form;
                        ...
                        else if (!string.IsNullOrEmpty(this.Request.Form["Go_Home_action"]) && this.Request.Form["Go_Home_action"].ToString().Equals("true", StringComparison.OrdinalIgnoreCase))
                        {
                            ...
                            form = SaveCurrentForm(form, surveyInfoModel, SurveyAnswer, responseId, UserId, IsSubmited, IsSaved, IsMobileDevice, FormValuesHasChanged, PageNumber, FormsHierarchy);
                            SaveCurrentFormToDocumentDB(DocumentForm, surveyInfoModel, SurveyAnswer, responseId, UserId, IsSubmited, IsSaved, FormValuesHasChanged, PageNumber);
                        ...
                        else if (!string.IsNullOrEmpty(this.Request.Form["Get_Child_action"]) && this.Request.Form["Get_Child_action"].ToString().Equals("true", StringComparison.OrdinalIgnoreCase))
                        {
                            ...
                            form = SaveCurrentForm(form, surveyInfoModel, SurveyAnswer, responseId, UserId, IsSubmited, IsSaved, IsMobileDevice, FormValuesHasChanged, PageNumber, FormsHierarchy, responseId);
                            form = SetLists(form);
                            TempData["Width"] = form.Width + 5;
        ...
        private void SetGlobalVariable()
        {
            ...
            this.RequiredList = (Session[SessionKeys.RequiredList] ?? string.Empty).ToString();
            bool.TryParse((Session[SessionKeys.IsEditMode] ?? "False").ToString(), out this.IsEditMode);
        ...
        private FormResponseInfoModel GetFormResponseInfoModel(string SurveyId, string ResponseId, List<Epi.Web.Enter.Common.DTO.FormsHierarchyDTO> FormsHierarchyDTOList = null)
        {
            ...
            if (!string.IsNullOrEmpty(SurveyId))
            {
                ...
                var FieldDigest = formResponseInfoModel.ColumnDigests.ToDictionary(c => c.Key, c => c.Value);

                var ChilRecords = _isurveyDocumentDBStoreFacade.GetChildRecordByChildFormId(SurveyId, ResponseId, formSettingResponse.FormInfo.FormName, FieldDigest);

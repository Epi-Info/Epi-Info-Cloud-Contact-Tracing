...
namespace Epi.Core.EnterInterpreter.Rules
{
    public class Rule_UnHighlight :EnterRule
    {
           ...
           // this.Context.EnterCheckCodeInterface.UnHighlight(this.IdentifierList, this.IsExceptList);
            ...
            if (!this.IsExceptList)
            {
                foreach (string s in this.IdentifierList)
                {
                    ...
                    {
                        this.Context._HighlightedFieldList.Remove(s.ToLower());
                    }
                }
            }
            else
            {
                ...
                foreach (EpiInfo.Plugin.IVariable v in this.Context.CurrentScope.FindVariables(EpiInfo.Plugin.VariableScope.DataSource))
                {
                    ...
                    {
                        this.Context._HighlightedFieldList.Remove(key);
                    }
                }
        ...
        public override void ToJavaScript(StringBuilder pJavaScriptBuilder)
        {
            ...
            if (this.IsExceptList)
            {
                pJavaScriptBuilder.AppendLine("CCE_UnHighlight(List,true);");
            }
            else
            {
                pJavaScriptBuilder.AppendLine("CCE_UnHighlight(List,false);");
            }

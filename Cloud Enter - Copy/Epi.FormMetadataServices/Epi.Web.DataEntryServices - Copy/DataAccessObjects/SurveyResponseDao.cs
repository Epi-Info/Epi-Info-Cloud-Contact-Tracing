    ...
    public class SurveyResponseDao : MetadataAccessor, ISurveyResponseDao
    {
        ...
        public List<SurveyResponseBO> GetSurveyResponse(List<string> surveyResponseIdList, Guid userPublishKey, int gridgridPageNumber = -1, int gridgridPageSize = -1)
        {
            ...
            if (surveyResponseIdList.Count > 0)
            {
                foreach (string surveyResponseId in surveyResponseIdList.Distinct())
                {
                    ...
                    using (var Context = Web.EF.DataObjectFactory.CreateContext())
                    {
                        ...
                        var response = Context.SurveyResponses.FirstOrDefault(x => x.ResponseId == Id);
                        result.Add(Mapper.Map(new SurveyResponse(), null, lastActiveUserId));
        ...
        public void InsertSurveyResponse(SurveyResponseBO surveyResponseBO)
        {
            try
            {
                ...
                request.Criteria.FormName = formDigest.FormName;
        ...
        public List<SurveyResponseBO> GetFormResponseByFormId(SurveyAnswerCriteria criteria)
        {
            ...
            try
            {
                ...
                {
                    var gridFieldDigests = criteria.FieldDigestList ?? new Dictionary<int, FieldDigest>();
                    
                    var dbRespone = _surveyDocumentDBStoreFacade.ReadAllRecordsBySurveyID(criteria.FormName, gridFieldDigests);
                    IEnumerable<SurveyResponse> recordsInDocumentDB = (IEnumerable<SurveyResponse>)dbRespone;
                    if (recordsInDocumentDB != null)
                    {
                        var responseList = recordsInDocumentDB.Skip((criteria.PageNumber - 1) * criteria.PageSize).Take(criteria.PageSize);
                        foreach (SurveyResponse response in responseList)
                        {
        ...
        public List<SurveyResponseBO> GetResponsesHierarchyIdsByRootId(string rootResponseId)
        {
            ...
            try
            {
                Guid Id = new Guid(rootResponseId);

                // TODO temporary until the DocumentDB implimentation is complete.
                result.Add(new SurveyResponseBO { ResponseId = rootResponseId });
        ...
        public int GetDataAccessRule(string formId, int userId)
        {
            ...
            if (formDigest != null) ruleId = formDigest.DataAccessRuleId;
        ...
        public bool DoesResponseExist(Guid ResponseId)
        {
            var eweResult = _eweEntitySurveyResponseDao.DoesResponseExist(ResponseId);
            return eweResult;
            throw new NotImplementedException();
        }
